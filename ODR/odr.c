...
void handle_rreq(char* pkt, int local_ifindex, char* canonical_addr, int net_sockfd)
{
  ...
  {
    printf("handle_rreq:Hop Count Exceeded %d, dropping the RREQ\n", MAX_HOPCOUNT);
    return;
  }
  ...
  {
    printf("handle_rreq: Looped back RREQ received.. dropping it\n");
    return;
  }
  ...
  {
    printf("handle_rreq: Force Discovery Flag set [RREQ] message.\n");
  ...
  if (route_entry == NULL)
  {
    printf("handle_rreq: No route for %s, adding it\n", rreq_msg.src_addr);
  ...
  {
    /* Honour only monotonosly increasing broadcast IDs from a client */
    if(rreq_msg.broadcast_id >= route_entry->broadcast_id)
    {
      ...
      {
        printf("handle_rreq: Better Route found for %s, updating it\n", rreq_msg.src_addr);
        del_route_entry(rreq_msg.src_addr, &route_hash_table);
        add_route_entry(rreq_msg.src_addr, neighbor_mac, rreq_msg.hop_count,
            local_ifindex, curr_time, rreq_msg.broadcast_id, &route_hash_table);
      }
      ...
      {
        /* Do not flood these RREQs out, just refresh the timestamp for duplicate RREQs */
        route_entry->timestamp = curr_time;
        rreq_msg.hop_count = route_entry->numhops;  
        printf("handle_rreq: rreq_msg.broadcast_id = %d, route_entry->broadcast_id = %d\n", rreq_msg.broadcast_id, route_entry->broadcast_id);
	if (rreq_msg.broadcast_id == route_entry->broadcast_id)
           need_forward = FALSE;
      }
    }
  ...
  if(need_forward)
  {
	...
	return;
      ...
      else
      {
        ...
        if (dst_route)
        {
          printf("handle_rreq: Route found in handle_rreq, sending RRESP\n");
...
void handle_rresp(char* pkt, int local_ifindex, char* canonical_addr, int net_sockfd)
{
  ...
  {
    printf("handle_rresp: Force Discovery Flag set [RREP] message.\n");
  ...
  /* If Stale, remove it..  */
  curr_time = get_current_seconds();
  if (route_entry &&  ((curr_time - route_entry->timestamp) >= stale_timeout))
  {
  ...
  else
  {
    ...
    if(rresp_msg.broadcast_id >= route_entry->broadcast_id)
    {
      ...
      {
        /* Do not forward these RREPs out, just refresh the timestamp for duplicate RREPs*/
        route_entry->timestamp = curr_time;
        rresp_msg.hop_count = route_entry->numhops;  
	if (rresp_msg.broadcast_id == route_entry->broadcast_id)
           need_forward = FALSE;
      }
    }
  ...
  if(need_forward)
  {
    ...
    /* If not found initiate a RREQ, unlikely case */
    if (!dst_route)
    {
      /* Send RREQ */
...
void handle_payload(char* pkt, int local_ifindex, char* canonical_addr, int net_sockfd, int domain_sockfd)
{
  ...
  else
  {
      ...
      else 
      {
        ...
        payload_msg->hop_count = route_entry->numhops;  
        route_entry->timestamp = curr_time;
      }
  }
  ...
  while (1) {
    ...
    if (FD_ISSET(domain_sockfd, &all))
    {
      ...
      if ((entry = find_path_port_from_path(&path_port_list, client_addr.sun_path)))
      {
        ...
        printf("[Info] : Found Client, associated port = %d\n", client_port);
        /* Update Freshness */
      ...
      else
      {
        ...
        printf("[Info] : New Client, associate port = %d\n", client_port);
      ...
      if (0 == strcmp(recv_msg.dst_addr, canonical_addr))
      {
        ...
        if (recv_msg.flag)
        {
          printf("[Info] : Client %s sending rreq with destination %s and force discovery flag\n", 
        ...
        if (route_entry)
        {
          ...
          else 
          {
            ...
            printf("[Info] : Stale Route found for  %s, removing it..\n", recv_msg.dst_addr);
            del_route_entry(recv_msg.dst_addr, &route_hash_table);
          }
        }
    ...
    if (FD_ISSET(net_sockfd, &all)) {
      ...
      switch(msg_type)
      {
        ...
        case MSG_PAYLOAD: 
          ...
          break;
        default:
          printf("[Info] : Unknown Message, Ignoring\n");
      }
    }
  }
  return 0;
