...
void sig_child_handler(int signo)
{
  ...
  snprintf(msg, MAX_LINE, "[Info] Child terminated, PID: [%d]\n", valid_pid);
    ...
    int *eof, congestion_ctrl_info_t *cc_info)
{
  ...
  {
    printf(" Buffer is not empty, cc_info->sendwin = %d\n", cc_info->sendwin);
  ...
  for( ; i < cc_info->sendwin; i++)
  {
    ...
    printf("[send_file_conet]: seq = %u, pkt_info->seq = %u\n", seq, pkt_info->seq);
    ...
    printf("[send_file_conet]:  slot = %d, send_buf->next_read_seq = %u, send_buf->next_contig_write_seq = %u\n", slot, send_buf->next_read_seq, send_buf->next_contig_write_seq);
    /* TODO: Check for buffer full */
    //assert(send_buf->next_contig_write_seq - send_buf->next_contig_write_seq
...
int file_transfer(int sockfd, FILE *f, server_config_t *config, unsigned int init_seq)
{
  ...
  printf("[Initialize] send_buf->next_read_seq = send_buf->next_contig_write_seq = %u\n", init_seq);
...
send_data:
    ...
    if (sigsetjmp(jmpbuf_data, 1) != 0) {
      ...
      /* TODO: Inform Congestion Control */
      congest_occur(&cc_info, CONGEST_TIMEOUT);
      goto send_data;
    }
    ...
    while (1)
    {
      ...
      /* TODO: Print the Client IP as well using getpeername */
      printf("[Info] Received ACK [%u] from client [%s]\n", pkt_info->ack, "<client_ip>");
      ...
      {
        /* TODO: Inform Congestion Control */ 
        transmit_occur(&cc_info);

        /* TODO:   Adjust Send window */
        ...
        {
          /* TODO: Inform Congestion Control */ 
          congest_occur(&cc_info, CONGEST_DUPACK);
          
          printf("[Info] Fast retransmit packet with Seq [%du\n", next_to_send->seq);
    ...
    struct sockaddr_in *cli_addr, packet_info_t *in_pkt_info, server_config_t *config) 
{
  ...
  /* TODO: Check whether on the same host or subnet */
#if 0
  else if (ipaddr_on_subnet(client_ip, ifi_array[listenfd_idx]->subnet))
  {
...
sendagain:
  ...
  /* Update RTO */
  printf("rtt_ts(&rttinfo) = %u,  rcv_pkt_info->timestamp = %u", rtt_ts(&rttinfo), rcv_pkt_info->timestamp);
...
int main(int argc, char **argv)
{
  ...
  char rcv_line[MAX_LINE], client_id[ADDR_LEN], tmp[ADDR_LEN];
  short client_port;
  ...
  for(;;)
  {
    ...
    for (i = 0; i < num_ifi; i++) 
    {
      if (FD_ISSET(ifi_array[i]->sockfd, &rset)) 
      {
        ...
        if ((n = recvfrom(ifi_array[i]->sockfd, rcv_line, MAX_LINE, 0, 
