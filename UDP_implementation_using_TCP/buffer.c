...
int write_to_buffer(circ_buffer_t *buf, packet_info_t *pkt_info)
{
  ...
  printf("  rcv_buf->next_read_seq = %u, rcv_buf->next_contig_write_seq = %u\n", buf->next_read_seq,
      buf->next_contig_write_seq);
  return 0;
}
...
int read_from_buffer(circ_buffer_t *buf, packet_info_t **entry)
{
  ...
  printf("Next slot to read = %d for seq number %u\n", slot, buf->next_read_seq);
  assert(*entry = buf->slots[slot]);
 
  printf("In read_from_buffer:\n");
  printf("  Next slot to read = %d for seq number %u\n", slot, buf->next_read_seq);
  print_packet_info(*entry);
...
int read_upto_seq(circ_buffer_t *buf, unsigned int seq, packet_info_t **entry)
{
  ...
  *entry = NULL;
  ...
  {
    printf("  send_buf->next_read_seq = %u, send_buf->next_contig_write_seq = %u\n", buf->next_read_seq,
      buf->next_contig_write_seq);
    /* Find the next Entry that can be read */
    slot = SEQ_TO_SLOT(buf->next_read_seq, buf);
    assert(*entry = buf->slots[slot]);
