...
int server_on_same_subnet(char* ip,ifi_t *ifi_array[], int total_ifi, char* cli_ip)
{	
  ...
  for(i=0;i<total_ifi;i++)
  {
	...
	{
	  same_host = 1;
	  printf("[Info] server on same host\n");
	  strcpy(cli_ip,"127.0.0.1");
	}
    inet_pton(AF_INET,ifi_array[i]->nmask,&mask_int);
    inet_pton(AF_INET,ip,&ip_int);
    mask_int = (ip_int & (mask_int));
    inet_ntop(AF_INET,&mask_int,temp[i],IP_MAX);
    //printf("n/w part of i/p ip for each iface: %s, naddr=%s\n",temp[i],ifi_array[i]->subnet);
    //n = strcmp(temp[i],ifi_array[i]->naddr);
  ...
  if (i == total_ifi){
    printf("[Info]server ip doesn't match any local n/w\n");
    return 0;
  }
    ...
    if(!same_host){
    		...
    		//printf("longest-prefix matched:%s\n",ifi_array[j]->ip_addr);
